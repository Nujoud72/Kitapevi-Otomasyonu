...
typedef struct Kitap //KÝTAP YAPISI ÝÇÝNDE DEÐÝÞKEN TUTMAYA YARIYOR
{
	...
	char kitapAdi[30]; 
	char yazarAdi[30];                                                        //kÝTÝPHENEYE KÝTAP EKLENMESÝ DURUMUNDA BU YAPIYI KULLANARAK ALACAÐIZ
	char kitapTuru[30];
	int sayfaSayisi;
} kitap ;//YUKARIDAKÝ BÝLGÝLERÝ kitap ÝSÝMLÝ DEÐÝÞKENLE ?A?IRALIM 
...
} uye ;//YUKARIDAKÝ BÝLGÝLERÝ uye ÝSÝMLÝ DEÐÝÞKENLE ?A?IRALIM
...
} emanet ;//YUKARIDAKÝ BÝLGÝLERÝ emanet ÝSÝMLÝ DEÐÝÞKENLE ?A?IRALIM

void kitapEkle()
{
	system("cls");//EKRAN TEMÝZLER YENÝ EKRAN ATAR
	...
	FILE * ptr = fopen("kitaplar.dat" , "a+b");//kitaplar.dat ADLI DOSYA ATAR. DOSYAYA VERÝ EKLEMEK ÝÇÝN a+b modu kullanýlýr.
	fwrite( &k1, sizeof(kitap), 1 , ptr);//OLUÞTURULAN DOSYAYA KÝTAP YAPISINDA TANIMLADIÐIMIZ DEÐÝÞKENLERÝ YAZAR KAYDEDER.
	fclose(ptr);//ATILAN DOSYAYI KAPATIR
...
void kitapListele()
{
	...
	fclose(ptr);//ATILAN DOSYAYI KAPATI
...
void uyeEkle()
{
	...
	//DOSYAYA VERÝ EKLEMEK AMACIYA BU KOD SATIRLARINI KULLANACAÐIZ
	FILE * ptr = fopen("uyeler.dat" , "a+b");//kitaplar.dat ADLI DOSYA ATAR. DOSYAYA VERÝ EKLEMEK ÝÇÝN a+b modu kullanýlýr.
...
int main()
{
	...
	while( secim != 0 )
	{
		...
		secim = menu();//MENÜ FONKSÝYONUNU SÜREKL?Ý ?A?IRSIN
	}
